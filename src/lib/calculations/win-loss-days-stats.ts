...
export function calculateWinLossDaysStats(trades: TradeForStats[]): {
  winning: WinLossDaySideStats;
  losing: WinLossDaySideStats;
} {
  // Group trades by day, then classify days as winning/losing
  const dayMap: Record<string, TradeForStats[]> = {};
  for (const t of trades) {
    const day = format(t.entryDate, "yyyy-MM-dd");
  ...
  const winningDayTrades: TradeForStats[] = [];
  const losingDayTrades: TradeForStats[] = [];
  let winDayCount = 0;
  let lossDayCount = 0;

  for (const [, dayTrades] of Object.entries(dayMap)) {
    const dayPnl = dayTrades.reduce((s, t) => s + t.pnl, 0);
    if (dayPnl > 0) {
      winDayCount++;
      winningDayTrades.push(...dayTrades);
    } else if (dayPnl < 0) {
      lossDayCount++;
      losingDayTrades.push(...dayTrades);
    }
    // Break-even days (dayPnl === 0) are excluded from both sides
  }

  return {
    winning: computeSideStats(winningDayTrades, winDayCount),
    losing: computeSideStats(losingDayTrades, lossDayCount),
  };
}
